  ...
  const fetchDashboardData = useCallback(async (forceRefresh = false) => {
    ...
    try {
      ...
      const [todayRes, monthRes, userRes] = await Promise.allSettled([
        fetch(`http://localhost:3000/api/attendance/day?date=${dateInfo.today}`, {
          headers: { Authorization: `Bearer ${token}` }
        }),
        fetch(`http://localhost:3000/api/attendance/summary?start=${dateInfo.monthStart}&end=${dateInfo.monthEnd}`, {
          headers: { Authorization: `Bearer ${token}` }
        }),
        refreshUser()
      ]);
