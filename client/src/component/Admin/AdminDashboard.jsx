  ...
  const fetchStats = async () => {
    try {
      ...
      const attRes = await fetch(
        `http://localhost:3000/api/attendance?startDate=${today}&endDate=${today}`,
        { headers: { Authorization: `Bearer ${token}` } }
      );
      ...
      const breakfast = todayAttendance.filter(
        a => a.meal_type === "breakfast" && a.status === "will_attend"
      ).length;

      const lunch = todayAttendance.filter(
        a => a.meal_type === "lunch" && a.status === "will_attend"
      ).length;

      const dinner = todayAttendance.filter(
        a => a.meal_type === "dinner" && a.status === "will_attend"
      ).length;
