...
const Menu = () => {
  ...
  useEffect(() => {
    const fetchSelectedPackage = async () => {
      try {
        const response = await api('/api/menu/selected-package');

        if (response.ok || response.success) {
          setSelectedPackage(response.selectedPackage);
        }
      } catch (error) {
  ...
  const handlePackageSelect = async (packageId) => {
    ...
    try {
      const response = await api('/api/menu/select-package', {
        method: 'POST',
      ...
      if (response.status === 200) {
        ...
        toast({
          ...
          description: `You have selected the ${menuPackages.find(p => p.id === packageId)?.name}. Your monthly bill has been updated to ₹${response.monthlyBill}.`,
        });
        ...
        toast({
          title: 'Error',
          description: response.error || 'Failed to select package',
          variant: 'destructive',
        });
      }
    } catch (error) {
