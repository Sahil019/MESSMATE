...
const Payment = () => {
  ...
  const fetchPaymentData = async () => {
    try {
      // Fetch QR code
      const qrData = await api('/api/payment/qr');
      setQrCode(qrData.qrCode);

      // Fetch billing info
      const monthStart = format(startOfMonth(new Date()), 'yyyy-MM-dd');
      const monthEnd = format(endOfMonth(new Date()), 'yyyy-MM-dd');
      const billingData = await api(`/api/billing/summary?start=${monthStart}&end=${monthEnd}`);
